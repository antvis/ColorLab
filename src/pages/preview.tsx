...
const Preview = () => {
  ...
  const fitDashboards = DASHBOARDS.filter((dashboard) => dashboard.type === currentPalette.type);
  const defaultDashboardName = fitDashboards[0].name;
  const [dashboardName, setDashboardName] = useState<string>(defaultDashboardName);
  ...
  if (!isContinuousPalette(currentPalette) && !isMatrixPalette(currentPalette)) {
    return (
      <div className={classNames(styles.container, styles.preview)}>
        <div className={styles.header}>
          ...
          {!isNormal && (
            <Optimization
              ...
              optimize={() => setCurrentPalette(paletteOptimization(currentPalette, { locked, simulationType }), false)}
            ></Optimization>
          )}
          <div className={styles.dashboardConfigPanel}>
            <Select value={dashboardName} style={{ width: 120 }} onChange={handleDashboardChange}>
        ...
        <div className={styles.content} style={{ height: isNormal ? 'calc(100% - 165px)' : 'calc(100% - 275px)' }}>
          <Dashboard
            dashborad={fitDashboards.find((dashboard) => dashboard.name === dashboardName)}
            palette={currentPalette}
            simulationType={simulationType}
