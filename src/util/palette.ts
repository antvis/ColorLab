...
import { colorToHex, SimulationType, colorSimulation } from '@antv/smart-color';
...
export function getSimulationColors(palette: Palette, type: SimulationType): string[] {
  if (!isContinuousPalette(palette) && !isMatrixPalette(palette)) {
    return palette.colors.map((color: Color) => colorToHex(colorSimulation(color, type)));
  }
  return [];
}
